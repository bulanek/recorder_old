...
#define __enable_peripheral(p)	(&RCC->AHB1ENR)[p/32] |= 1 << (p % 32)
#define __disable_peripheral(p)	(&RCC->AHBENR)[p/32] &= ~(1 << (p % 32))
...
#define FCLK_FAST() { SPIx_CR1 = (SPIx_CR1 & ~0x38) | 0x00; }	/* Set SCLK = PCLK / 2 */
...
#if SPI_CH == 3	/* PA4:MMC_CS, PA5:MMC_SCLK, PA6:MMC_DO, PA7:MMC_DI, PC4:MMC_CD */
    #define CS_HIGH()	GPIOA->BSRR = _BV(15)
    #define CS_LOW()	GPIOA->BSRR = _BV(15+16)
    #define	MMC_CD		!(GPIOC->IDR & _BV(15))	/* Card detect (yes:true, no:false, default:true) */
    ...
    #define	SPIxENABLE() {\
        ...
        SPIx_CR1 = _BV(9)|_BV(8)|_BV(6)|_BV(2);					/* Enable SPI3 */\
}\


#else
    #error "Wrong SPI used (SPI_CH)"
#endif
